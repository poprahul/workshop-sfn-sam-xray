...
```bash
sam-app-sfn$ sam build --use-container
```
...
```bash
sam-app-sfn$ sam logs -n StockCheckerFunction --stack-name sam-app-sfn --tail
```
...
```bash
sam-app-sfn$ pip install pytest pytest-mock --user
sam-app-sfn$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name sam-app-sfn
```
